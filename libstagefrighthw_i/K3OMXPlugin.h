...
struct K3OMXComponent {
    OMX_COMPONENTTYPE *mComponent;
    K3OMXCore *mCore;
};

struct K3OMXPlugin : public OMXPluginBase {
    K3OMXPlugin();
    virtual ~K3OMXPlugin();
...
private:
    ...
    Vector<K3OMXCore*> mCores;
    Vector<K3OMXComponent> mComponents;

    OMX_ERRORTYPE AddCore(const char* coreName);

    K3OMXPlugin(const K3OMXPlugin &);
    K3OMXPlugin &operator=(const K3OMXPlugin &);
};
