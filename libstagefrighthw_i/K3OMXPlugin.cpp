...
{
   AddCore("lib_k3_omxcore.so");
}

OMX_ERRORTYPE K3OMXPlugin::AddCore(const char* coreName)
{
   void* libHandle = dlopen(coreName, RTLD_NOW);

   if (libHandle != NULL) {
        K3OMXCore* core = (K3OMXCore*)calloc(1,sizeof(K3OMXCore));
        ...
        core->mInit = (K3OMXCore::InitFunc)dlsym(libHandle, "OMX_Init");
        core->mDeinit = (K3OMXCore::DeinitFunc)dlsym(libHandle, "OMX_Deinit");

        core->mComponentNameEnum =
            (K3OMXCore::ComponentNameEnumFunc)dlsym(libHandle, "OMX_ComponentNameEnum");

        core->mGetHandle = (K3OMXCore::GetHandleFunc)dlsym(libHandle, "OMX_GetHandle");
        core->mFreeHandle = (K3OMXCore::FreeHandleFunc)dlsym(libHandle, "OMX_FreeHandle");

        core->mGetRolesOfComponentHandle =
            (K3OMXCore::GetRolesOfComponentFunc)dlsym(
...
OMX_ERRORTYPE K3OMXPlugin::makeComponentInstance(
        const char *name,
...
OMX_ERRORTYPE K3OMXPlugin::destroyComponentInstance(
        OMX_COMPONENTTYPE *component) {
    Mutex::Autolock autoLock(mMutex);
...
OMX_ERRORTYPE K3OMXPlugin::enumerateComponents(
        OMX_STRING name,
        size_t size,
        OMX_U32 index) {
...
OMX_ERRORTYPE K3OMXPlugin::getRolesOfComponent(
        const char *name,
